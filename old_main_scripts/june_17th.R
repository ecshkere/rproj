...
###### in vitro RNA-seq
ASE_in_vitro <- function(patient, FDR = 0.05, min_DP = 50) {
...
ase_in_vitro <- lapply(patients_vtr, function(x) {
  ASE_in_vitro(x, FDR = 0.05, min_DP = 50)
})
...
# 3520 генов с diffASE при FDR = 0.05 и min_DP = 50
ase %>% filter(diffASE == TRUE) %>% dplyr::select(SYMBOL) %>% unique() %>% nrow()

# из них 1797 с SNP в промоторах
ase <- ase %>%
...
# 3920 генов с SNP в промоторах общих у 2+ людей для которых есть SNP в экзонах
...
# для каждого гена из RNA-seq находим какие SNP из чипсеков (общие у 2+ людей) есть в промоторе этого гена у данного человека
df <- merge(ase_filtered, joined_df_filt, by = c("SYMBOL", "patient"), suffixes = c(".RNA", ".CS"), all.y = TRUE) %>%
  dplyr::select(SYMBOL, patient, diffASE, log2FC_1vs0, p01adj, chr.RNA, pos.RNA, ref.RNA, alt, DP_0R, DP_1R, rsid) %>%
...
# 1177 SNP в чипсеках общих у 2+ людей в промоторах генов из RNA-seq
length(unique(df[df$diffASE == TRUE, ]$rsid)) 

# 1 ген такой чтобы у двух людей было diffASE и одинаковые SNP в промоторах
rsids <- df %>% na.omit() %>% dplyr::count(rsid) %>% dplyr::filter(n > 1) %>% dplyr::select(rsid) %>% .[[1]] %>% unique()
length(rsids)
View(df %>% filter(rsid %in% rsids) %>% na.omit())
...
df <- merge(ase_filtered, joined_df_filt, by = "SYMBOL", suffixes = c(".RNA", ".CS"), all.y = TRUE) %>%
  dplyr::select(SYMBOL, patient.RNA, diffASE, log2FC_1vs0, p01adj, chr.RNA, pos.RNA, ref.RNA, alt, DP_0R, DP_1R, rsid, patient.CS) %>%
  mutate(p01adj = round(p01adj, 2), log2FC_1vs0 = round(log2FC_1vs0, 2)) %>%
  # оставляем гены для которых хотя бы у одного человека есть diffASE
  group_by(SYMBOL) %>% filter(any(diffASE == TRUE)) %>% ungroup()

# 534 ген такой чтобы было diffASE хотя бы у одного человека и совпадающие SNP в промоторах хотя бы у двух
length(unique(df$SYMBOL))
length(unique(df$rsid)) # 1579 SNP

df %>%
  group_by(rsid) %>%
  filter(n_distinct(patient.RNA) >= 2) %>%
  ungroup() %>%
  View()
