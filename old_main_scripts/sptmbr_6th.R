...
run_mbased_patient <- function(ase_df, patient_id, ref_cols = c("DP_ref_0R", "DP_ref_1R"), alt_cols = c("DP_alt_0R", "DP_alt_1R")) {
  
  ase_sub <- ase_df %>% filter(patient == patient_id)
  snps <- GRanges(
  ...
  # Loop over timepoints (0 and 1)
  for (i in seq_along(ref_cols)) {
    timepoint <- as.character(i - 1) # 0 or 1
    ref_count <- ref_cols[i]
    alt_count <- alt_cols[i]
    col_name <- paste0(tolower(patient_id), timepoint, "R")
    
    # Create SummarizedExperiment
    se <- SummarizedExperiment(
      assays = list(
        lociAllele1Counts = matrix(ase_sub[[ref_count]], ncol = 1, dimnames = list(names(snps), col_name)),
        lociAllele2Counts = matrix(ase_sub[[alt_count]], ncol = 1, dimnames = list(names(snps), col_name))
      ),
      rowRanges = snps
    )
    ...
    gene_output <- summarizeASEResults(results)$geneOutput
