...
ase_df_for_mbased <- ase %>%
  group_by(SYMBOL, patient) %>%
  filter(any(p01 < 0.01)) %>%
  ungroup()

# Function to run MBASED for a single patient
run_mbased_patient <- function(ase_df, patient_id) {
  
  ase_sub <- ase_df_for_mbased %>% filter(patient == patient_id)
  snps <- GRanges(
  ...
  se <- SummarizedExperiment(
    assays = list(
      lociAllele1Counts = matrix(c(ase_sub[["DP_ref_0R"]], ase_sub[["DP_ref_1R"]]), ncol = 2, dimnames = list(names(snps), c("0", "1"))),
      lociAllele2Counts = matrix(c(ase_sub[["DP_alt_0R"]], ase_sub[["DP_alt_1R"]]), ncol = 2, dimnames = list(names(snps), c("0", "1")))
    ),
    rowRanges = snps
  )
  bp <- MulticoreParam(workers = 48)
  
  start_time <- Sys.time()
  results <- runMBASED(ASESummarizedExperiment = se, isPhased = FALSE, numSim = 10^6, BPPARAM = bp)
  end_time <- Sys.time()
  ...
  # patient_id <- "A"
  ase_df <- ase_df_for_mbased %>% filter(patient == patient_id)
  all_results <- list()  # Initialize list to store results
  counter <- 0
  total <- length(unique(ase_df$SYMBOL))
  for (gene in unique(ase_df$SYMBOL)) {

    
    ase_sub <- ase_df %>% filter(SYMBOL == gene)
    print(nrow(ase_sub))
    if (nrow(ase_sub) > 20) {next}
    snps <- GRanges(
      seqnames = ase_sub$chr,
      ranges   = IRanges(ase_sub$pos, width = 1),
      aseID    = ase_sub$SYMBOL,
    ...
    results <- runMBASED(ASESummarizedExperiment = se, isPhased = FALSE, numSim = 10^6, BPPARAM = SerialParam())
    end_time <- Sys.time()
    print(end_time - start_time)
  ...
  genes <- unique(ase_df$SYMBOL)
  
  # Параллельные параметры: укажи число ядер
  param <- MulticoreParam(workers = 40)
  
  all_results <- bplapply(
    genes,
    function(gene) {
      ase_sub <- ase_df %>% filter(SYMBOL == gene)
      ...
      se <- SummarizedExperiment(
        assays = list(
          lociAllele1Counts = matrix(c(ase_sub[["DP_ref_0R"]], ase_sub[["DP_ref_1R"]]),
                                     ncol = 2,
                                     dimnames = list(names(snps), c("0", "1"))),
          lociAllele2Counts = matrix(c(ase_sub[["DP_alt_0R"]], ase_sub[["DP_alt_1R"]]),
                                     ncol = 2,
                                     dimnames = list(names(snps), c("0", "1")))
        ),
        rowRanges = snps
      ...
      runMBASED(ASESummarizedExperiment = se,
                isPhased = FALSE,
                numSim   = 10^6)
    },
    BPPARAM = param
  ...
  gene_output$timepoint <- timepoint
  gene_output$SYMBOL <- rownames(gene_output)
  
  # Store results
  patient_results[[timepoint]] <- gene_output
  locus_results[[timepoint]] <- data.frame(
    snp_id = names(unlist(summarizeASEResults(results)$locusOutput)),
    as.data.frame(unlist(summarizeASEResults(results)$locusOutput), row.names = NULL)
  ) %>%
    merge(gene_output[, c("SYMBOL", "majorAlleleFrequency")], by.x = "aseID", by.y = "SYMBOL")
  
  # Merge locus results with original ASE table and compute mbased_lfc
  df0 <- locus_results[["0"]]
  df1 <- locus_results[["1"]]
  full_df <- merge(df0, ase_sub, by.x = c("seqnames", "start", "aseID"), by.y = c("chr", "pos", "SYMBOL"))
  full_df <- merge(full_df, df1, by = c("seqnames", "start", "aseID"))
  full_df <- full_df %>%
    mutate(absLFC = abs(ifelse(allele1IsMajor.x == allele1IsMajor.y,
...
patients <- c("D", "A", "B")
full_mbased <- bind_rows(lapply(patients, function(p) run_mbased_patient(ase, p))) %>%
