...
########### SNP из чипсеков
dm <- read_tsv("/media/leon/DISK2/icig/diabet/merge/Damarov.vcf")
an <- read_tsv("/media/leon/DISK2/icig/diabet/merge/E.Viacheslavovna.vcf")
bn <- read_tsv("/media/leon/DISK2/icig/diabet/merge/N.Petrovna.vcf")
...
snpsDm <- GRanges(seqnames = aseDm$chr, ranges = IRanges(aseDm$pos, width = 1), aseID = aseDm$SYMBOL, allele1 = aseDm$ref,allele2 = aseDm$alt)
names(snpsDm) <- paste(aseDm$SYMBOL, aseDm$variant, sep = "_")
dm0R <- SummarizedExperiment(assays = list(lociAllele1Counts = matrix(aseDm$DP_ref_0R, ncol = 1,dimnames = list(names(snpsDm), "dm0R")), lociAllele2Counts = matrix(aseDm$DP_alt_0R, ncol = 1, dimnames = list(names(snpsDm), "dm0R"))), rowRanges = snpsDm)
...
dm1R <- SummarizedExperiment(assays = list(lociAllele1Counts = matrix(aseDm$DP_ref_1R, ncol = 1, dimnames = list(names(snpsDm), "dm1R")), lociAllele2Counts = matrix(aseDm$DP_alt_1R, ncol = 1, dimnames = list(names(snpsDm), "dm1R"))), rowRanges = snpsDm)
...
an0R <- SummarizedExperiment(assays = list(lociAllele1Counts = matrix(aseAn$DP_ref_0R, ncol = 1, dimnames = list(names(snpsAn), "an0R")), lociAllele2Counts = matrix(aseAn$DP_alt_0R, ncol = 1, dimnames = list(names(snpsAn), "an0R"))), rowRanges = snpsAn)
...
an1R <- SummarizedExperiment(assays = list(lociAllele1Counts = matrix(aseAn$DP_ref_1R, ncol = 1, dimnames = list(names(snpsAn), "an1R")), lociAllele2Counts = matrix(aseAn$DP_alt_1R, ncol = 1, dimnames = list(names(snpsAn), "an1R"))), rowRanges = snpsAn)
...
bn0R <- SummarizedExperiment(assays = list(lociAllele1Counts = matrix(aseBn$DP_ref_0R, ncol = 1, dimnames = list(names(snpsBn), "bn0R")), lociAllele2Counts = matrix(aseBn$DP_alt_0R, ncol = 1, dimnames = list(names(snpsBn), "bn0R"))), rowRanges = snpsBn)
...
bn1R <- SummarizedExperiment(assays = list(lociAllele1Counts = matrix(aseBn$DP_ref_1R, ncol = 1, dimnames = list(names(snpsBn), "bn1R")), lociAllele2Counts = matrix(aseBn$DP_alt_1R, ncol = 1, dimnames = list(names(snpsBn), "bn1R"))), rowRanges = snpsBn)
...
dm_full <- dm_full %>% # отношение аллеля, преобладающего в точке 1, к альтернативному
...
result <- df %>%
  ...
  filter(
    ...
    diffdiffASE                # либо diffASE у обоих но в разную сторону ### не учитываем
  ) %>% ungroup() %>%
