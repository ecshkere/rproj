...
ddmaf_df <- inner_join(ase, distinct(df_collapsed[, c("symbol", "patients")])) %>%
  ...
  filter(n == max(n)) %>%
  filter(total_across_dp == min(total_across_dp)) %>%
...
length(unique(ddmaf_df[1:112, ]$symbol)) # 101
ddmaf_df[[111, "dd_ref_frac"]] # 0.201  # 0.143773
ddmaf_df_filt <- ddmaf_df[1:111, ] %>% 
...
# all_degs001 <- sort(unique(TF_deseq_results$symbol))
# length(all_degs) # 118
length(intersect(read_tsv("output/newer_list_of_degtfs.txt", col_names = F)$X1, all_degs))
setdiff(read_tsv("output/newer_list_of_degtfs.txt", col_names = F)$X1, all_degs005)
setdiff(all_degs001, read_tsv("output/newer_list_of_degtfs.txt", col_names = F)$X1)
